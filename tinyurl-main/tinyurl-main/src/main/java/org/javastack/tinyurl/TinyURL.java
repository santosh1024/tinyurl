...
public class TinyURL extends HttpServlet {
	...
	private void doPost0(final HttpServletRequest request, final HttpServletResponse response)
		...
		// Optional custom id parameter (allows creating a short path like /r/NV6Mtr)
		String customId = request.getParameter("id");
		if (customId != null) {
			// Validate format using same rules as getPathInfoKey
			if (customId.isEmpty()) {
				sendError(response, out, HttpServletResponse.SC_BAD_REQUEST, "Invalid id parameter");
				return;
			}
			for (int i = 0; i < customId.length(); i++) {
				final char c = customId.charAt(i);
				if ((c >= 'A') && (c <= 'Z'))
					continue;
				if ((c >= 'a') && (c <= 'z'))
					continue;
		...
		if ((customId != null) && !customId.isEmpty()) {
			key = customId;
			final TinyData existing = store.get(key);
			if (existing != null) {
				// If same URL, reuse; otherwise error
				if (url.equals(existing.getURL())) {
					sendResponse(request, response, out, url, key, collision, false);
					return;
				} else {
					sendError(response, out, HttpServletResponse.SC_CONFLICT, "ID already exists");
					return;
				}
			}
		} else {
			key = hasher.hashURL(url);
		...
		// Store new URL
		store.put(key, url);
		sendResponse(request, response, out, url, key, collision, true);
	}
	...
	private final String getBaseURL(final HttpServletRequest request) {
		...
		final String scheme = getRequestScheme(request);
		final String host = getRequestHost(request);
	...
	private final String getRedirectBaseURL(final HttpServletRequest request) {
	...
	private final void sendResponse(final HttpServletRequest request, final HttpServletResponse response,
			final PrintWriter out, final String url, final String key, final int collision, final boolean isNew) {
		final String res = "{ \"id\": \"" + key + "\", \"short_url\": \"" + getJSONSafe(getRedirectBaseURL(request) + key) + "\" }";
	...
	private static final String getJSONSafe(final String value) {
